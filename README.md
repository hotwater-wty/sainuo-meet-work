...
`v0.2` 的 P1–P5 和本地浏览器验收已完成；P5.5 正在进行体验稳定化：产品支持来源导入、可回退的阅读画像路由、文档地图、阶段精读、可信引用、确认式笔记和 Markdown 导出。P6 公网部署与 P7 面试材料尚未开始。
...
建议验收路径：导入 MyBatis 规范 → 选择阅读画像 → 生成路线 → 开始首阶段 → 打开来源 → 结束阶段 → 生成并编辑草稿 → 接受 → 导出笔记。当前本地实测为 57 页、80 个来源块、4 阶段路线；刷新后会恢复 TTL 内会话并定位到下一个待开始阶段。`/home`、`/profile`、`/reading` 分别对应导入、画像和阅读工作区，调整画像后可使用浏览器后退返回路线。
