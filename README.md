...
`v0.2` 的 P1–P5 和本地浏览器验收已完成；P5.5 正在进行体验稳定化：产品支持门面首页、分阶段路由、来源导入、可回退的阅读画像路由、文档地图、阶段精读、可信引用、确认式笔记和 Markdown 导出。P6 公网部署与 P7 面试材料尚未开始；Redis 方案暂缓，当前仍为匿名进程内临时会话。
...
- `docs/reports/ROUTING_REVIEW.md`：门面首页、业务路径与阶段二级路由的职责边界。
...
浏览器访问 [http://localhost:3000](http://localhost:3000)。`/` 是产品门面首页，点击“立即体验”进入 `/home` 文件导入；后续 `/profile`、`/reading`、`/reading/stage` 分别对应阅读画像、文档地图/路线和阶段精读。当前 Demo 使用获准的 MyBatis PDF 文件上传；会话最后活动 2 小时后清理，开发服务器重启后允许丢失。

建议验收路径：从门面首页点击“立即体验” → 导入 MyBatis 规范 → 选择阅读画像 → 生成路线 → 开始首阶段 → 打开来源 → 结束阶段 → 生成并编辑草稿 → 接受 → 导出笔记。路线未完成时可点击“暂时离开”，确认后清理当前匿名会话并返回门面首页；全部阶段完成后点击“学学别的”返回门面首页。当前本地实测为 57 页、80 个来源块、4 阶段路线；刷新后会恢复 TTL 内会话并定位到下一个待开始阶段。
